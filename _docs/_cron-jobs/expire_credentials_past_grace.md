...
Transitions `user_credentials.credential_status` from `PENDING_REVIEW` / `VERIFIED` to `EXPIRED` once `grace_expires_at` has passed. Sends the expired credential email when it changes the status.
